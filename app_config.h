#ifndef APP_CONFIG_H
#define APP_CONFIG_H

#include <stdint.h>

#define MOTOR_PWM_PERIOD        (1600U)
#define MOTOR_PWM_DEAD_TICKS    (20U)
#define SPEED_CTRL_PERIOD_MS    (20U)
#define UART_REPORT_PERIOD_MS   (100U)
#define PWM_STEP_LIMIT          (30)
#define STARTUP_SAFE_MS         (800U)

#define DEFAULT_TARGET_COUNTS   (120.0f)
#define DEFAULT_KP              (1.20f)
#define DEFAULT_KI              (0.003f)
#define DEFAULT_KD              (0.0f)
#define DEFAULT_MAX_OUTPUT      (390.0f)
#define DEFAULT_MAX_INTEGRAL    (270.0f)

#define DEFAULT_LEFT_PWM_TRIM   (0)
#define DEFAULT_RIGHT_PWM_TRIM  (-14)
#define DEFAULT_BRAKE_MS        (120U)

#define LINE_DEFAULT_KP         (3.80f)
#define LINE_DEFAULT_KD         (2.40f)
#define LINE_DEFAULT_TARGET     (190.0f)
#define LINE_DEFAULT_BASE_PWM   (400)
#define LINE_CORR_LIMIT         (460)
#define LINE_MIN_WHEEL_TARGET   (25.0f)
#define LINE_SEARCH_PWM         (0)
#define LINE_CORRECTION_SIGN    (-1)
#define LINE_CURVE_ERR_START    (14)
#define LINE_CURVE_TARGET_DROP  (55.0f)

#define Q4_AB_TARGET_TIME_MS     (7000U)
/* 模式二先越过电机死区，再在 2.5s 内从 200 线性增加到 400。 */
#define Q4_ACCEL_TIME_MS         (2500U)
#define Q4_CRUISE_TARGET_COUNTS  (150.0f)
#define Q4_START_PWM             (200)
#define Q4_CRUISE_BASE_PWM       (400)
/* 与模式一相同的启动变化率，使输出约 140ms 到达起步 PWM。 */
#define Q4_PWM_STEP_LIMIT        (30)
#define Q4_LINE_PWM_LIMIT        (180)
#define Q4_LEFT_PWM_TRIM         (0)
#define Q4_RIGHT_PWM_TRIM        (-14)
#define Q4_BRAKE_MS              (120U)
#define Q4_LINE_KP               (5.20f)
#define Q4_LINE_KD               (3.00f)
#define Q4_LINE_CORR_LIMIT       (240)
#define Q4_LINE_SEARCH_PWM       (0)
#define Q4_LINE_CURVE_ERR_START  (15)
#define Q4_LINE_CURVE_PWM_DROP   (70)
#define Q4_LINE_CURVE_TARGET_DROP (70.0f)
#define Q4_BALL_TARGET_MM        (0.0f)
#define Q4_EST_CRUISE_SPEED_MPS  (0.248f)
#define Q4_GRAVITY_MPS2          (9.80665f)
#define Q4_MAX_FEEDFORWARD_DEG   (3.0f)
/*
 * 编码器首次确认车轮已经运动后立即介入，并在 60ms 内快速渐入。
 * 这样补偿与实车起步接近同步，同时避免直接给水管角度阶跃。
 */
#define Q4_FEEDFORWARD_ENABLE    (1)
#define Q4_INITIAL_ANGLE_DEG     (1.70f)
/* 启动时固定保持明确的目标倾角；到时直接交给稳定版 PID，不做渐退。 */
#define Q4_INITIAL_ANGLE_HOLD_MS (100U)
#define Q4_PRELOAD_LEAD_MS       (100U)
#define Q4_INITIAL_ANGLE_RATE_DEG_S (240.0f)
/* 实机确认原移动补偿方向相反：仅翻转加速度前馈，不翻转球位置反馈。 */
#define Q4_FEEDFORWARD_SIGN      (1.0f)
#define Q4_ENCODER_METERS_PER_COUNT (0.00003333f)
#define Q4_ACCEL_LIMIT_MPS2      (0.45f)
#define Q4_MEASURED_ACCEL_WEIGHT (0.80f)
#define Q4_MOVING_SPEED_CPS      (50)

/* 模式二中心保持专用位置 PID；不影响模式一和要求3。 */
#define Q4_BALL_POS_KP           (0.080f)
#define Q4_BALL_POS_KI           (0.004f)
#define Q4_BALL_POS_KD           (0.000f)
#define Q4_BALL_POS_WEIGHT       (0.65f)
#define Q4_BALL_SPEED_KP         (0.050f)
#define Q4_BALL_SPEED_KI         (0.002f)
#define Q4_BALL_SPEED_KD         (0.00020f)
#define Q4_BALL_SPEED_WEIGHT     (0.35f)
#define Q4_BALL_PID_DEADBAND_MM  (2.0f)
#define Q4_BALL_PID_INT_LIMIT    (80.0f)
#define Q4_BALL_SPEED_INT_LIMIT  (100.0f)
#define Q4_BALL_ACCEL_LIMIT_MM_S2 (1200.0f)
#define Q4_BALL_MEDIAN_DELAY_FRAMES (1U)
#define Q4_BALL_ACTUATOR_LOOKAHEAD_MS (20U)
#define Q4_BALL_LOOKAHEAD_MAX_MS (120U)
#define Q4_BALL_PREDICT_POS_MAX_MM (18.0f)
#define Q4_BALL_PREDICT_SPEED_DELTA_MAX (80.0f)
#define Q4_BALL_POSITION_FILTER_NEW (0.45f)
#define Q4_BALL_VELOCITY_FILTER_NEW (0.50f)
#define Q4_BALL_ACCEL_FILTER_NEW (0.25f)
#define Q4_BALL_POS_OUT_MAX_DEG  (2.0f)
#define Q4_BALL_SPEED_OUT_MAX_DEG (1.5f)
#define Q4_BALL_PID_MAX_THETA    (1.8f)
#define Q4_A_MARK_TOTAL           (3U)
#define Q4_A_MARK_SPAN            (3U)
#define Q4_A_MARK_FRAMES          (2U)
#define Q4_A_MIN_LAP_MS           (5000U)
#define Q4_A_MIN_LAP_COUNTS       (9000U)

/*
 * 模式三（题目要求6）独立参数副本。初值与模式二完全相同，但禁止写成
 * Q4_* 的宏别名，确保后续实车整定任意位置保持时不会改变模式二。
 */
#define Q6_LAP_TARGET_TIME_MS     (30000U)
#define Q6_ACCEL_TIME_MS          (4000U)
#define Q6_CRUISE_TARGET_COUNTS   (150.0f)
#define Q6_START_PWM              (200)
#define Q6_CRUISE_BASE_PWM        (400)
#define Q6_PWM_STEP_LIMIT         (30)
#define Q6_LINE_PWM_LIMIT         (180)
#define Q6_LEFT_PWM_TRIM          (0)
#define Q6_RIGHT_PWM_TRIM         (-14)
#define Q6_BRAKE_MS               (120U)
#define Q6_LINE_KP                (5.20f)
#define Q6_LINE_KD                (3.00f)
#define Q6_LINE_CORR_LIMIT        (240)
#define Q6_LINE_SEARCH_PWM        (0)
#define Q6_LINE_CURVE_ERR_START   (15)
#define Q6_LINE_CURVE_PWM_DROP    (70)
#define Q6_LINE_CURVE_TARGET_DROP (70.0f)
#define Q6_EST_CRUISE_SPEED_MPS   (0.248f)
#define Q6_MAX_FEEDFORWARD_DEG    (3.0f)
#define Q6_FEEDFORWARD_ENABLE     (1)
#define Q6_INITIAL_ANGLE_DEG      (1.70f)
#define Q6_INITIAL_ANGLE_HOLD_MS  (300U)
#define Q6_BALL_FEEDBACK_BLEND_MS (400U)
#define Q6_PRELOAD_LEAD_MS        (100U)
#define Q6_INITIAL_ANGLE_RATE_DEG_S (240.0f)
#define Q6_FEEDFORWARD_SIGN       (1.0f)
/* 模式三入弯纵向减速前馈；提前介入、快速建立、平滑退出。 */
#define Q6_CURVE_FF_ENABLE        (1)
/* 起步最初 800ms 复用模式二行为；车身稳定后及时开放首个弯道增强。 */
#define Q6_CURVE_ARM_DELAY_MS     (800U)
#define Q6_CURVE_FF_ENTRY_ERR     (15)
#define Q6_CURVE_FF_ENTRY_CONFIRM_MS (80U)
#define Q6_CURVE_FF_EXIT_ERR      (4)
#define Q6_CURVE_FF_FULL_ERR      (24)
#define Q6_CURVE_FF_MAX_DEG       (0.75f)
#define Q6_CURVE_FF_HOLD_DEG      (0.45f)
#define Q6_CURVE_FF_SIGN          (-1.0f)
#define Q6_CURVE_FF_ATTACK_NEW    (0.65f)
#define Q6_CURVE_FF_RELEASE_NEW   (0.25f)
#define Q6_CURVE_FF_MIN_HOLD_MS   (1000U)
#define Q6_CURVE_FF_EXIT_CONFIRM_MS (300U)
/* 模式三弯道内增益调度：位置略增强，速度阻尼明显增强，积分适当减弱。 */
#define Q6_CURVE_CASCADE_POS_KP   (1.50f)
#define Q6_CURVE_CASCADE_POS_KI   (0.015f)
#define Q6_CURVE_CASCADE_POS_KD   (0.10f)
#define Q6_CURVE_CASCADE_POS_INT_LIMIT (60.0f)
#define Q6_CURVE_CASCADE_TARGET_SPEED_MAX_MM_S (55.0f)
#define Q6_CURVE_CASCADE_SPEED_KP (0.040f)
#define Q6_CURVE_CASCADE_SPEED_KI (0.0010f)
#define Q6_CURVE_CASCADE_SPEED_KD (0.00015f)
#define Q6_CURVE_CASCADE_SPEED_INT_LIMIT (80.0f)
#define Q6_CURVE_CASCADE_ANGLE_MAX_DEG (2.00f)
#define Q6_CURVE_CASCADE_DEADBAND_MM (1.0f)
/* +x points to the hinge end; shift only the mode-3 curve target that way. */
#define Q6_CURVE_TARGET_BIAS_MM    (5.0f)

/* Mode-3 straight cascade PID, independent from the proven curve settings. */
#define Q6_STRAIGHT_CASCADE_POS_KP (0.65f)
#define Q6_STRAIGHT_CASCADE_POS_KI (0.003f)
#define Q6_STRAIGHT_CASCADE_POS_KD (0.12f)
#define Q6_STRAIGHT_CASCADE_POS_INT_LIMIT (30.0f)
#define Q6_STRAIGHT_CASCADE_TARGET_SPEED_MAX_MM_S (22.0f)
#define Q6_STRAIGHT_CASCADE_SPEED_KP (0.055f)
#define Q6_STRAIGHT_CASCADE_SPEED_KI (0.0003f)
#define Q6_STRAIGHT_CASCADE_SPEED_KD (0.00005f)
#define Q6_STRAIGHT_CASCADE_SPEED_INT_LIMIT (40.0f)
#define Q6_STRAIGHT_CASCADE_ANGLE_MAX_DEG (1.20f)
#define Q6_STRAIGHT_CASCADE_DEADBAND_MM (2.0f)
/* +x is the hinge end; compensate mode-3 straight equilibrium only. */
#define Q6_STRAIGHT_TARGET_BIAS_MM (8.0f)

/* Mode-3-only static-friction breakaway pulse near the target. */
#define Q6_STICTION_ENABLE             (1)
#define Q6_STICTION_ERROR_ENTER_MM     (10.0f)
#define Q6_STICTION_ERROR_EXIT_MM      (5.0f)
#define Q6_STICTION_SPEED_ENTER_MM_S   (8.0f)
#define Q6_STICTION_SPEED_RELEASE_MM_S (20.0f)
#define Q6_STICTION_STABLE_MS          (300U)
#define Q6_STICTION_PULSE_MS           (180U)
#define Q6_STICTION_COOLDOWN_MS        (300U)
#define Q6_STICTION_BOOST_DEG          (0.15f)

/* Mode-3 visual mean-error learner, independent for straight and curve. */
#define Q6_ADAPTIVE_BIAS_ENABLE              (1)
#define Q6_ADAPTIVE_BIAS_DEADBAND_MM         (3.0f)
#define Q6_ADAPTIVE_BIAS_MAX_MM              (12.0f)
#define Q6_ADAPTIVE_BIAS_RATE_PER_S          (0.10f)
#define Q6_ADAPTIVE_BIAS_MEAN_TAU_MS         (800U)
#define Q6_ADAPTIVE_BIAS_QUALIFY_MS          (800U)
#define Q6_ADAPTIVE_BIAS_POSITION_WINDOW_MM  (40.0f)
#define Q6_ADAPTIVE_BIAS_SPEED_MAX_MM_S      (80.0f)

/* Mode-3 straight acceleration loop: fast visual observer plus latency lead. */
#define Q6_VISUAL_ACCEL_FB_ENABLE             (1)
#define Q6_VISUAL_ACCEL_FB_KP_DEG_PER_MM_S2   (0.0010f)
#define Q6_VISUAL_ACCEL_FB_MAX_DEG            (0.45f)
#define Q6_VISUAL_ACCEL_FB_SIGN               (1.0f)
#define Q6_VISUAL_ACCEL_OBSERVER_NEW          (0.70f)
#define Q6_VISUAL_ACCEL_JERK_LIMIT_MM_S3      (3000.0f)
#define Q6_VISUAL_ACCEL_PREDICT_MAX_MS        (80U)
#define Q6_VISUAL_ACCEL_INPUT_LIMIT_MM_S2     (500.0f)

/* Immediate chassis-acceleration feedforward; 180/(pi*g) is about 5.84. */
#define Q6_DRIVE_ACCEL_FF_GAIN_DEG_PER_MPS2   (5.84f)
#define Q6_DRIVE_ACCEL_FF_MAX_DEG             (0.50f)
#define Q6_DRIVE_ACCEL_FF_SIGN                (1.0f)

#define Q6_BALL_POS_KP            (0.080f)
#define Q6_BALL_POS_KI            (0.004f)
#define Q6_BALL_POS_KD            (0.000f)
#define Q6_BALL_POS_WEIGHT        (0.65f)
#define Q6_BALL_SPEED_KP          (0.050f)
#define Q6_BALL_SPEED_KI          (0.002f)
#define Q6_BALL_SPEED_KD          (0.00020f)
#define Q6_BALL_SPEED_WEIGHT      (0.35f)
#define Q6_BALL_PID_DEADBAND_MM   (2.0f)
#define Q6_BALL_PID_INT_LIMIT     (80.0f)
#define Q6_BALL_SPEED_INT_LIMIT   (100.0f)
#define Q6_BALL_ACCEL_LIMIT_MM_S2 (1200.0f)
#define Q6_BALL_MEDIAN_DELAY_FRAMES (1U)
#define Q6_BALL_ACTUATOR_LOOKAHEAD_MS (20U)
#define Q6_BALL_LOOKAHEAD_MAX_MS  (120U)
#define Q6_BALL_PREDICT_POS_MAX_MM (18.0f)
#define Q6_BALL_PREDICT_SPEED_DELTA_MAX (80.0f)
#define Q6_BALL_POSITION_FILTER_NEW (0.45f)
#define Q6_BALL_VELOCITY_FILTER_NEW (0.50f)
#define Q6_BALL_ACCEL_FILTER_NEW  (0.25f)
#define Q6_BALL_POS_OUT_MAX_DEG   (2.0f)
#define Q6_BALL_SPEED_OUT_MAX_DEG (1.5f)
#define Q6_BALL_PID_MAX_THETA     (1.8f)
#define Q6_A_MARK_TOTAL           (3U)
#define Q6_A_MARK_SPAN            (3U)
#define Q6_A_MARK_FRAMES          (2U)
#define Q6_A_MIN_LAP_MS           (5000U)
#define Q6_A_MIN_LAP_COUNTS       (9000U)

/*
 * Mode 4 (Q7): arbitrary non-center target.  Keep a complete parameter copy
 * so later Q7 tuning cannot change the proven mode-3 behavior.
 */
#define Q7_LAP_TARGET_TIME_MS     (30000U)
#define Q7_ACCEL_TIME_MS          (4000U)
#define Q7_CRUISE_TARGET_COUNTS   (150.0f)
#define Q7_START_PWM              (200)
#define Q7_CRUISE_BASE_PWM        (400)
#define Q7_PWM_STEP_LIMIT         (30)
#define Q7_LINE_PWM_LIMIT         (180)
#define Q7_LEFT_PWM_TRIM          (0)
#define Q7_RIGHT_PWM_TRIM         (-14)
#define Q7_BRAKE_MS               (120U)
#define Q7_LINE_KP                (5.20f)
#define Q7_LINE_KD                (3.00f)
#define Q7_LINE_CORR_LIMIT        (240)
#define Q7_LINE_SEARCH_PWM        (0)
#define Q7_LINE_CURVE_ERR_START   (15)
#define Q7_LINE_CURVE_PWM_DROP    (70)
#define Q7_LINE_CURVE_TARGET_DROP (70.0f)
#define Q7_EST_CRUISE_SPEED_MPS   (0.248f)
#define Q7_MAX_FEEDFORWARD_DEG    (3.0f)
#define Q7_FEEDFORWARD_ENABLE     (1)
#define Q7_INITIAL_ANGLE_DEG      (1.70f)
#define Q7_INITIAL_ANGLE_HOLD_MS  (300U)
#define Q7_BALL_FEEDBACK_BLEND_MS (400U)
#define Q7_PRELOAD_LEAD_MS        (100U)
#define Q7_INITIAL_ANGLE_RATE_DEG_S (240.0f)
#define Q7_FEEDFORWARD_SIGN       (1.0f)

#define Q7_CURVE_FF_ENABLE        (1)
#define Q7_CURVE_ARM_DELAY_MS     (800U)
#define Q7_CURVE_FF_ENTRY_ERR     (15)
#define Q7_CURVE_FF_ENTRY_CONFIRM_MS (80U)
#define Q7_CURVE_FF_EXIT_ERR      (4)
#define Q7_CURVE_FF_FULL_ERR      (24)
#define Q7_CURVE_FF_MAX_DEG       (0.75f)
#define Q7_CURVE_FF_HOLD_DEG      (0.45f)
#define Q7_CURVE_FF_SIGN          (-1.0f)
#define Q7_CURVE_FF_ATTACK_NEW    (0.65f)
#define Q7_CURVE_FF_RELEASE_NEW   (0.25f)
#define Q7_CURVE_FF_MIN_HOLD_MS   (1000U)
#define Q7_CURVE_FF_EXIT_CONFIRM_MS (300U)

#define Q7_CURVE_CASCADE_POS_KP   (1.50f)
#define Q7_CURVE_CASCADE_POS_KI   (0.015f)
#define Q7_CURVE_CASCADE_POS_KD   (0.10f)
#define Q7_CURVE_CASCADE_POS_INT_LIMIT (60.0f)
#define Q7_CURVE_CASCADE_TARGET_SPEED_MAX_MM_S (55.0f)
#define Q7_CURVE_CASCADE_SPEED_KP (0.040f)
#define Q7_CURVE_CASCADE_SPEED_KI (0.0010f)
#define Q7_CURVE_CASCADE_SPEED_KD (0.00015f)
#define Q7_CURVE_CASCADE_SPEED_INT_LIMIT (80.0f)
#define Q7_CURVE_CASCADE_ANGLE_MAX_DEG (2.00f)
#define Q7_CURVE_CASCADE_DEADBAND_MM (1.0f)
/* Mode 4 needs slightly more hinge-side correction than mode 3. */
#define Q7_CURVE_TARGET_BIAS_MM   (10.0f)

#define Q7_STRAIGHT_CASCADE_POS_KP (0.65f)
#define Q7_STRAIGHT_CASCADE_POS_KI (0.003f)
#define Q7_STRAIGHT_CASCADE_POS_KD (0.12f)
#define Q7_STRAIGHT_CASCADE_POS_INT_LIMIT (30.0f)
#define Q7_STRAIGHT_CASCADE_TARGET_SPEED_MAX_MM_S (22.0f)
#define Q7_STRAIGHT_CASCADE_SPEED_KP (0.055f)
#define Q7_STRAIGHT_CASCADE_SPEED_KI (0.0003f)
#define Q7_STRAIGHT_CASCADE_SPEED_KD (0.00005f)
#define Q7_STRAIGHT_CASCADE_SPEED_INT_LIMIT (40.0f)
#define Q7_STRAIGHT_CASCADE_ANGLE_MAX_DEG (1.20f)
#define Q7_STRAIGHT_CASCADE_DEADBAND_MM (2.0f)
#define Q7_STRAIGHT_TARGET_BIAS_MM (13.0f)

/* Mode-4 copy: independent macros allow later mode-specific tuning. */
#define Q7_STICTION_ENABLE             (1)
#define Q7_STICTION_ERROR_ENTER_MM     (10.0f)
#define Q7_STICTION_ERROR_EXIT_MM      (5.0f)
#define Q7_STICTION_SPEED_ENTER_MM_S   (8.0f)
#define Q7_STICTION_SPEED_RELEASE_MM_S (20.0f)
#define Q7_STICTION_STABLE_MS          (300U)
#define Q7_STICTION_PULSE_MS           (180U)
#define Q7_STICTION_COOLDOWN_MS        (300U)
#define Q7_STICTION_BOOST_DEG          (0.15f)

/* Mode-4 copy; learns around the K230 arbitrary-position target. */
#define Q7_ADAPTIVE_BIAS_ENABLE              (1)
#define Q7_ADAPTIVE_BIAS_DEADBAND_MM         (3.0f)
#define Q7_ADAPTIVE_BIAS_MAX_MM              (12.0f)
#define Q7_ADAPTIVE_BIAS_RATE_PER_S          (0.10f)
#define Q7_ADAPTIVE_BIAS_MEAN_TAU_MS         (800U)
#define Q7_ADAPTIVE_BIAS_QUALIFY_MS          (800U)
#define Q7_ADAPTIVE_BIAS_POSITION_WINDOW_MM  (40.0f)
#define Q7_ADAPTIVE_BIAS_SPEED_MAX_MM_S      (80.0f)

/* Mode-4 independent copy of the straight acceleration loop. */
#define Q7_VISUAL_ACCEL_FB_ENABLE             (1)
#define Q7_VISUAL_ACCEL_FB_KP_DEG_PER_MM_S2   (0.0010f)
#define Q7_VISUAL_ACCEL_FB_MAX_DEG            (0.45f)
#define Q7_VISUAL_ACCEL_FB_SIGN               (1.0f)
#define Q7_VISUAL_ACCEL_OBSERVER_NEW          (0.70f)
#define Q7_VISUAL_ACCEL_JERK_LIMIT_MM_S3      (3000.0f)
#define Q7_VISUAL_ACCEL_PREDICT_MAX_MS        (80U)
#define Q7_VISUAL_ACCEL_INPUT_LIMIT_MM_S2     (500.0f)

/* Mode-4 independent chassis-acceleration feedforward. */
#define Q7_DRIVE_ACCEL_FF_GAIN_DEG_PER_MPS2   (5.84f)
#define Q7_DRIVE_ACCEL_FF_MAX_DEG             (0.50f)
#define Q7_DRIVE_ACCEL_FF_SIGN                (1.0f)

/* State-7 LCD target-bias tuning envelope; +x points to hinge end. */
#define Q67_TARGET_BIAS_TUNE_MIN_MM (-30.0f)
#define Q67_TARGET_BIAS_TUNE_MAX_MM (30.0f)

#define Q7_BALL_POS_KP            (0.080f)
#define Q7_BALL_POS_KI            (0.004f)
#define Q7_BALL_POS_KD            (0.000f)
#define Q7_BALL_POS_WEIGHT        (0.65f)
#define Q7_BALL_SPEED_KP          (0.050f)
#define Q7_BALL_SPEED_KI          (0.002f)
#define Q7_BALL_SPEED_KD          (0.00020f)
#define Q7_BALL_SPEED_WEIGHT      (0.35f)
#define Q7_BALL_PID_DEADBAND_MM   (2.0f)
#define Q7_BALL_PID_INT_LIMIT     (80.0f)
#define Q7_BALL_SPEED_INT_LIMIT   (100.0f)
#define Q7_BALL_ACCEL_LIMIT_MM_S2 (1200.0f)
#define Q7_BALL_MEDIAN_DELAY_FRAMES (1U)
#define Q7_BALL_ACTUATOR_LOOKAHEAD_MS (20U)
#define Q7_BALL_LOOKAHEAD_MAX_MS  (120U)
#define Q7_BALL_PREDICT_POS_MAX_MM (18.0f)
#define Q7_BALL_PREDICT_SPEED_DELTA_MAX (80.0f)
#define Q7_BALL_POSITION_FILTER_NEW (0.45f)
#define Q7_BALL_VELOCITY_FILTER_NEW (0.50f)
#define Q7_BALL_ACCEL_FILTER_NEW  (0.25f)
#define Q7_BALL_POS_OUT_MAX_DEG   (2.0f)
#define Q7_BALL_SPEED_OUT_MAX_DEG (1.5f)
#define Q7_BALL_PID_MAX_THETA     (1.8f)
#define Q7_A_MARK_TOTAL           (3U)
#define Q7_A_MARK_SPAN            (3U)
#define Q7_A_MARK_FRAMES          (2U)
#define Q7_A_MIN_LAP_MS           (5000U)
#define Q7_A_MIN_LAP_COUNTS       (9000U)

/*
 * Mode 5 / requirement 3 is intentionally independent from Q4/Q6/Q7.
 * The three fixed stages and final PD values below are ported from the
 * supplied Q3_CODE/mspm0_ccs reference project.
 *
 * The reference driver used PIPE_ANGLE_SIGN=-1 while this project applies
 * BALL_ACTUATOR_ANGLE_SIGN later, so this private sign converts reference
 * logical angles without changing any other mode's actuator direction.
 */
#define Q3_MODE5_TARGET_PLUS_MM          (50.0f)
#define Q3_MODE5_TARGET_MINUS_MM         (-50.0f)
#define Q3_MODE5_REFERENCE_ANGLE_SIGN    (1.0f)
#define Q3_MODE5_OUT_ACCEL_ANGLE_DEG     (-1.50f)
#define Q3_MODE5_OUT_ACCEL_TIME_MS       (800U)
#define Q3_MODE5_TRANSFER_ANGLE_DEG      (1.20f)
#define Q3_MODE5_TRANSFER_TIME_MS        (1600U)
#define Q3_MODE5_FINAL_BRAKE_ANGLE_DEG   (-0.40f)
#define Q3_MODE5_FINAL_BRAKE_TIME_MS     (450U)
#define Q3_MODE5_OPEN_LOOP_MOTOR_DPS     (150.0f)

/* State-6 LCD tuning safety envelope; private to Tianmeng mode M5. */
#define Q3_MODE5_TUNE_ANGLE_MIN_DEG      (-2.50f)
#define Q3_MODE5_TUNE_ANGLE_MAX_DEG      (2.50f)
#define Q3_MODE5_TUNE_TIME_MIN_MS        (100U)
#define Q3_MODE5_TUNE_TIME_MAX_MS        (3000U)
#define Q3_MODE5_TUNE_TOTAL_MAX_MS       (4900U)

/* 0: 三段结束后保持0度纯开环；1: 恢复最终-5 cm闭环微调。 */
#define Q3_MODE5_FINAL_PID_ENABLE         (0)

/* Final -5 cm visual PD hold, also copied from the reference project. */
#define Q3_MODE5_HOLD_KP_DEG_PER_MM      (0.0180f)
#define Q3_MODE5_HOLD_KI_DEG_PER_MM_S    (0.0100f)
#define Q3_MODE5_HOLD_KD_DEG_PER_MM_S    (0.0022f)
#define Q3_MODE5_HOLD_MAX_ANGLE_DEG      (1.20f)
#define Q3_MODE5_HOLD_INTEGRAL_ZONE_MM   (30.0f)
#define Q3_MODE5_HOLD_INT_MAX_ANGLE_DEG  (0.35f)
#define Q3_MODE5_HOLD_MOTOR_DPS           (150.0f)

/* Mode-5-only visual filtering; no Q4/Q6/Q7 parameter is reused. */
#define Q3_MODE5_ACCEL_LIMIT_MM_S2        (1600.0f)
#define Q3_MODE5_POSITION_FILTER_NEW      (0.55f)
#define Q3_MODE5_VELOCITY_FILTER_NEW      (0.60f)
#define Q3_MODE5_ACCEL_FILTER_NEW         (0.30f)

#define A_MARK_DEFAULT_TOTAL    (3U)
#define A_MARK_DEFAULT_SPAN     (3U)
#define A_MARK_DEFAULT_FRAMES   (2U)
#define A_MARK_FINISH_TOTAL     (5U)
#define A_MARK_FINISH_SPAN      (3U)
#define A_MARK_RELEASE_FRAMES   (4U)
#define A_MARK_MIN_LAP_MS       (5000U)
#define A_MARK_MIN_LAP_COUNTS   (9000U)
#define A_MARK_BASELINE_ALPHA   (4U)
#define A_MARK_JUMP_TOTAL       (1U)
#define A_MARK_JUMP_SPAN        (1U)
#define A_MARK_DEBUG_PERIOD_MS  (1000U)

#define BUTTON_REARM_MS         (200U)
#define OLED_RUNTIME_ENABLE     (1)
#define CONTROL_LAG_WARN_MS     (60U)
#define CONTROL_LAG_DEBUG_ENABLE (0)

#define LEFT_MOTOR_INVERT       (0)
#define RIGHT_MOTOR_INVERT      (0)
#define LEFT_ENCODER_INVERT     (0)
#define RIGHT_ENCODER_INVERT    (0)

/* X42S TTL 串口绝对位置控制参数，默认配合 X 固件使用。 */
#define X42S_ADDRESS            (1U)
#define X42S_STARTUP_DELAY_MS   (800U)
#define X42S_COMMAND_PERIOD_MS  (20U)
#define X42S_POSITION_POLL_MS   (100U)
#define X42S_STATUS_POLL_MS     (500U)
#define X42S_RX_TIMEOUT_MS      (300U)
#define X42S_POSITION_UNITS_PER_DEG (100.0f)
#define X42S_MOTOR_DEG_PER_PIPE_DEG (4.5f)
#define X42S_MAX_MOTOR_ANGLE_DEG (28.0f)
#define X42S_POSITION_TOLERANCE_DEG (0.20f)
#define STEPPER_DEFAULT_DPS     (90.0f)
#define STEPPER_MAX_DPS         (240.0f)
#define STEPPER_JOG_STEPS       (5)
#define STEPPER_JOG_INTERVAL_MS (80U)

#define K230_BALL_TIMEOUT_MS    (350U)
#define K230_BALL_MIN_CONF      (40)
#define K230_STATUS_PERIOD_MS   (100U)

#define BALL_CTRL_PERIOD_MS     (20U)
#define BALL_TARGET_PLUS_MM     (50.0f)
#define BALL_TARGET_MINUS_MM    (-50.0f)
#define BALL_SETTLE_ERR_MM      (8.0f)
#define BALL_SETTLE_SPEED_MM_S  (25.0f)
#define BALL_SETTLE_TICKS       (25U)
#define BALL_POS_KP             (2.0f)
#define BALL_POS_KD             (0.20f)
#define BALL_POS_MAX_SPEED      (140.0f)
#define BALL_SPEED_KP           (0.045f)
#define BALL_SPEED_KI           (0.040f)
#define BALL_SPEED_INT_MAX      (60.0f)
#define BALL_SPEED_MAX_THETA    (5.5f)
#define BALL_THETA_RATE_DEG     (90.0f)
#define BALL_ACTUATOR_ANGLE_SIGN (1.0f)

#endif
